...
main() {
    ...
    RELEASE_DIR="jiagubao-$RELEASE_VERSION"
    ...
    run tar zvcf "$RELEASE_FILE_NAME" "$RELEASE_DIR/bin/jiagubao" "$RELEASE_DIR/zsh-completion/_jiagubao"
    ...
    sed_in_place "/sha256   /c \  sha256   \"$RELEASE_FILE_SHA256SUM\"" Formula/jiagubao.rb
    sed_in_place "s@[0-9]\+\.[0-9]\+\.[0-9]\+@$RELEASE_VERSION@g"       Formula/jiagubao.rb

    run git add Formula/jiagubao.rb
    run git commit -m "'publish new version jiagubao $RELEASE_VERSION'"
